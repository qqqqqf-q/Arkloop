...
**用途**: Agent 管理。将现有 Agent Configs + Personas + Prompt Templates 合并为单一 "Agent" 概念。Arkloop 的 Agent 是配置态（非运行实例），不存在容器/心跳等概念。

### 数据映射

前端的一个 "Agent" = 后端的一个 Persona + 一个 Agent Config。

| Lite 字段 | 后端 Persona 字段 | 后端 Agent Config 字段 |
|---|---|---|
| Name | `display_name` | `name` |
| System Prompt | `prompt_md` | `system_prompt_override` |
| Model | `preferred_credential` | `model`（= credential name） |
| Tools | `tool_allowlist` | `tool_policy` + `tool_allowlist` |
| Is Active | `is_active` | - |
| Is Default | - | `is_default` |
| (Advanced) Temperature | - | `temperature` |
| (Advanced) Max Output Tokens | - | `max_output_tokens` |
| (Advanced) Reasoning Mode | - | `reasoning_mode` |

### 创建流程

1. 前端收集表单数据
2. `POST /v1/personas` 创建 Persona（`persona_key` 自动生成 slug，`version` 固定 "1.0"）
3. `POST /v1/agent-configs` 创建 Agent Config（`scope: "platform"`，`persona_id` 指向上一步创建的 Persona）
4. 前端将两者关联展示

### 列表视图

**API**:
- `GET /v1/personas` -> Persona 列表
- `GET /v1/agent-configs` -> Agent Config 列表
- 前端按 `persona_id` 关联
...
使用卡片布局，非表格。每张卡片仅显示：Name、Model、状态标签（Default/Active）、操作按钮。不在卡片上显示 Temperature、Tools 等细节。
...
| Model | select | Y | 主区域 | 下拉来源于 Models 页面的 LLM 条目（credential name） |
| System Prompt | textarea | Y | 主区域 | 直接文本，不支持模板变量 |
| Tools | checkbox group | N | 主区域 | 列出所有已激活的 Tool Provider |
| Is Default | checkbox | N | 主区域 | 设为默认 Agent |
...
**用途**: 统一管理 LLM 和 ASR 模型凭证。改名自 "LLM Credentials" + "ASR Credentials"。
...
| Name | text | Y | 凭证名称，唯一，也是 Agent 引用的标识符 |
...
```
1. POST /v1/llm-credentials
   body: {
     name,
     provider,        // 从 Client Type 映射
     api_key,
     base_url,
     openai_api_mode, // 从 Client Type 映射
     routes: [{
       model,         // Model Name 字段
       priority: 1,
       is_default: true,
...
**每个凭证自动创建一条 Route**。用户不感知 Route 概念。如果后续需要同一凭证下多个模型，在 Admin Console 中操作。

**编辑时**: `PATCH /v1/llm-credentials/{id}` 更新凭证字段，`PATCH /v1/llm-credentials/{id}/routes/{routeId}` 更新 Model Name。
...
│                                                 │
│  Base URL                                       │
│  [http://localhost:1933_______________]          │
│                                                 │
│  Root API Key                                   │
│  [••••••••••••••••••••••••]                     │
│                                                 │
...
│  Base URL        [http://localhost:8002_______]  │
...
| Section | Key | 类型 |
|---|---|---|
| General | `title_summarizer.agent_config_id` | UUID |
...
| Configuration | MCP Configs, Prompt Templates（合并到 Agents）, Agent Configs（合并到 Agents）, Personas（合并到 Agents） | 合并或不暴露 |
