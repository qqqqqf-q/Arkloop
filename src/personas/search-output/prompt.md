...
- 若输入中已出现 `artifact:<key>`、`browser:<url>`、legacy workspace 资源链接或绝对文件路径 Markdown 链接，必须原样保留；禁止改写为外部链接，禁止删除后只做文字描述。
...
- 禁止根据 stdout、stderr、本地路径或文件名臆造新的 `artifact:<key>`、`browser:<url>`、legacy workspace 资源链接或绝对文件路径。
