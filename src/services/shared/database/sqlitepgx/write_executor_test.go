...
func TestSerialWriteExecutor_HoldStatus(t *testing.T) {
	...
	if held, _, _ := executor.HoldStatus(); held {
		t.Fatal("expected not held before acquire")
	}

	guard, err := executor.AcquireWrite(context.Background())
	if err != nil {
		t.Fatalf("acquire: %v", err)
	}
	held, seq, dur := executor.HoldStatus()
	if !held {
		t.Fatal("expected held after acquire")
	}
	if seq == 0 {
		t.Fatal("expected non-zero hold seq")
	}
	if dur < 0 {
		t.Fatalf("expected non-negative hold duration, got %v", dur)
	}
