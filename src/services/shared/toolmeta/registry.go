...
var registry = []ToolMeta{
	...
	},
	{
		...
		LLMDescription: "execute browser automation commands in the isolated browser sandbox. Use browser only when web_search or web_fetch cannot complete the task because the page requires JavaScript rendering, DOM inspection, real interaction, login-flow reproduction, or multi-step tab navigation. Pass command as the raw agent-browser CLI subcommand, for example: navigate <url>, snapshot, screenshot, click <ref>, type <ref> <text>, fill <ref> <text>, press <key>, tab list, tab select <index>, console, network. Omit session_ref by default so the tool reuses the default browser session for the current run or thread. Pass session_ref only when you need to continue a specific existing browser session using the exact session_ref returned earlier. session_ref is a stable identifier, not a mode flag, so never pass placeholders or pseudo-modes such as new, resume, fork, or session_mode/share_scope fields. Recommended workflow: navigate to the page, use snapshot to inspect DOM/text and obtain fresh element refs, then interact with click/type/fill/press, and snapshot again after navigation or major UI changes because refs can become stale. Use screenshot only when you need a visual image; use snapshot when you need structure, text, or refs. For commands that may need the page to settle, especially navigate, snapshot after navigation, or interactions that trigger rendering, set yield_time_ms high enough to wait for a stable result; avoid tiny values such as 50ms, and prefer roughly 1500-5000ms unless the page is known to respond immediately. If a browser result returns running=true, that command is still in flight; do not issue another browser command on that same session_ref until it is no longer running. If the result includes artifacts, only reference artifact keys that actually appear there; never invent artifact keys or claim a screenshot exists when result.artifacts is empty.",
	},
	{
		Name:           "memory_search",
		Group:          GroupMemory,
