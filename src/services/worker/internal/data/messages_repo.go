...
func (MessagesRepository) ListRawByThread(
	ctx context.Context,
	tx pgx.Tx,
	accountID uuid.UUID,
	threadID uuid.UUID,
...
func scanThreadMessages(rows pgx.Rows) ([]ThreadMessage, error) {
	out := []ThreadMessage{}
	for rows.Next() {
		var item ThreadMessage
		if err := rows.Scan(
		...
		rows, err := tx.Query(
			...
			`SELECT m.id, m.role, m.content, m.content_json, m.metadata_json, m.created_at,
			  ...
			  WHERE m.account_id = $1
			    ...
			    AND m.thread_seq <= (SELECT thread_seq FROM upper_bound)
			  ORDER BY m.thread_seq ASC`,
			accountID,
			threadID,
			upToMessageID,
...
func (MessagesRepository) ListRawByThreadUpToID(
	ctx context.Context,
	tx pgx.Tx,
	accountID uuid.UUID,
	threadID uuid.UUID,
...
func (MessagesRepository) ListPendingAskUserFormsByRun(
	...
	rows, err := tx.Query(
		ctx,
		`SELECT id, role, content, content_json, metadata_json, created_at, thread_seq, NULL::bigint as output_tokens
		   FROM messages
		  WHERE account_id = $1
		    AND thread_id = $2
		    AND metadata_json->>'run_id' = $3
