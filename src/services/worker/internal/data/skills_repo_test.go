...
func TestSkillsRepositoryResolveEnabledSkills(t *testing.T) {
	...
	defer pool.Close()
	repo := NewSkillsRepository(pool)
	accountID := uuid.New()
	userID := uuid.New()
	profileRef := "pref_test"
	workspaceRef := "wsref_test"
	if _, err := pool.Exec(context.Background(), `INSERT INTO workspace_registries (workspace_ref, account_id, owner_user_id, metadata_json) VALUES ($1, $2, $3, '{}'::jsonb)`, workspaceRef, accountID, userID); err != nil {
		t.Fatalf("seed workspace registry: %v", err)
	}
	if _, err := pool.Exec(context.Background(), `INSERT INTO skill_packages (account_id, skill_key, version, display_name, instruction_path, manifest_key, bundle_key, files_prefix) VALUES ($1, 'grep-helper', '1', 'Grep Helper', 'SKILL.md', 'm1', 'b1', 'f1'), ($1, 'sed-helper', '1', 'Sed Helper', 'SKILL.md', 'm2', 'b2', 'f2')`, accountID); err != nil {
	...
	if _, err := pool.Exec(context.Background(), `INSERT INTO profile_skill_installs (profile_ref, account_id, owner_user_id, skill_key, version) VALUES ($1, $2, $3, 'grep-helper', '1')`, profileRef, accountID, userID); err != nil {
		t.Fatalf("seed profile install: %v", err)
	}
	if _, err := pool.Exec(context.Background(), `INSERT INTO workspace_skill_enablements (workspace_ref, account_id, enabled_by_user_id, skill_key, version) VALUES ($1, $2, $3, 'grep-helper', '1'), ($1, $2, $3, 'sed-helper', '1')`, workspaceRef, accountID, userID); err != nil {
		t.Fatalf("seed workspace enablement: %v", err)
	}
	if _, err := pool.Exec(context.Background(), `INSERT INTO profile_platform_skill_overrides (profile_ref, skill_key, version, status) VALUES ($1, 'builtin-manual', '1', 'manual'), ($1, 'builtin-removed', '1', 'removed')`, profileRef); err != nil {
...
func TestSkillsRepositoryResolveEnabledSkillsFallsBackToPlatformSkills(t *testing.T) {
	db := testutil.SetupPostgresDatabase(t, "worker_platform_skills")
	pool, err := pgxpool.New(context.Background(), db.DSN)
	if err != nil {
		t.Fatalf("new pool: %v", err)
	}
	...
	repo := NewSkillsRepository(pool)
	accountID := uuid.New()
	if _, err := pool.Exec(context.Background(), `INSERT INTO skill_packages (account_id, skill_key, version, display_name, instruction_path, manifest_key, bundle_key, files_prefix, sync_mode) VALUES (NULL, 'builtin-auto', '1', 'Builtin Auto', 'SKILL.md', 'pm1', 'pb1', 'pf1', 'platform_skill')`); err != nil {
