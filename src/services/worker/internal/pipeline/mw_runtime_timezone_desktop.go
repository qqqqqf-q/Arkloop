...
func runtimeContextTimeZone(ctx context.Context, rc *RunContext) string {
	...
	if rc.UserID != nil {
		...
		if err := rc.DB.QueryRow(ctx, `SELECT timezone FROM users WHERE id = $1 LIMIT 1`, *rc.UserID).Scan(&userTimeZone); err == nil {
	...
	if err := rc.DB.QueryRow(ctx, `SELECT timezone FROM accounts WHERE id = $1 LIMIT 1`, rc.Run.AccountID).Scan(&accountTimeZone); err == nil {
