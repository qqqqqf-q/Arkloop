...
func TestLoadRunInputsIncludesRoleFromFirstEvent(t *testing.T) {
	...
	threadID := uuid.New()
	runID := uuid.New()
	if _, err := pool.Exec(context.Background(), `INSERT INTO threads (id, org_id, project_id) VALUES ($1, $2, $3)`, threadID, orgID, projectID); err != nil {
		t.Fatalf("insert thread: %v", err)
	}
	if _, err := pool.Exec(context.Background(), `INSERT INTO runs (id, org_id, thread_id, status) VALUES ($1, $2, $3, 'running')`, runID, orgID, threadID); err != nil {
