...
func TestGeminiGateway_Stream_Provider413AddsOversizeDetails(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		http.Error(w, `{"error":{"code":413,"message":"too large","status":"REQUEST_TOO_LARGE"}}`, http.StatusRequestEntityTooLarge)
	}))
	defer server.Close()
