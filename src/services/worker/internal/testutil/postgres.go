...
func initRunsSchema(t *testing.T, dsn string) error {
	...
	statements := []string{
		...
		`CREATE TABLE thread_context_replacements (
			...
			start_context_seq BIGINT     NULL,
			end_context_seq   BIGINT     NULL,
			...
			CONSTRAINT chk_thread_context_replacements_context_range CHECK (
		...
		`CREATE TABLE replacement_supersession_edges (
			...
			created_at                TIMESTAMPTZ NOT NULL DEFAULT now(),
			CONSTRAINT chk_replacement_supersession_edges_target CHECK (
				num_nonnulls(superseded_replacement_id, superseded_chunk_id) = 1
			),
			CONSTRAINT chk_replacement_supersession_edges_no_self CHECK (
				superseded_replacement_id IS NULL OR superseded_replacement_id <> replacement_id
			),
