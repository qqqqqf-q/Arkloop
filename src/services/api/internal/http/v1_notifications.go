...
func listNotifications(
	...
	writeJSON(w, traceID, nethttp.StatusOK, map[string]any{"data": resp})
}
...
func markNotificationRead(
	...
	if err != nil {
		WriteError(w, nethttp.StatusUnprocessableEntity, "validation.error", "invalid notification id", traceID, nil)
		return
	}
	...
	if err := notifRepo.MarkRead(r.Context(), actor.UserID, notifID); err != nil {
		if err == pgx.ErrNoRows {
			WriteError(w, nethttp.StatusNotFound, "not_found", "notification not found or already read", traceID, nil)
			return
		}
		WriteError(w, nethttp.StatusInternalServerError, "internal_error", "failed to mark notification as read", traceID, nil)
		return
	}
