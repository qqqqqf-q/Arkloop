...
func RegisterRoutes(mux *nethttp.ServeMux, deps Deps) {
	...
	mux.HandleFunc("/v1/accounts/{id}/usage", accountUsageEntry(deps.AuthService, deps.AccountMembershipRepo, deps.UsageRepo, deps.APIKeysRepo))
	mux.HandleFunc("/v1/accounts/{id}/usage/daily", accountDailyUsage(deps.AuthService, deps.AccountMembershipRepo, deps.UsageRepo, deps.APIKeysRepo))
	mux.HandleFunc("/v1/accounts/{id}/usage/hourly", accountHourlyUsage(deps.AuthService, deps.AccountMembershipRepo, deps.UsageRepo, deps.APIKeysRepo))
	mux.HandleFunc("/v1/accounts/{id}/usage/by-model", accountUsageByModel(deps.AuthService, deps.AccountMembershipRepo, deps.UsageRepo, deps.APIKeysRepo))
	...
	mux.HandleFunc("/v1/me/usage/hourly", meHourlyUsage(deps.AuthService, deps.AccountMembershipRepo, deps.UsageRepo, deps.APIKeysRepo))
