...
func handleShareGet(
	...
	if share == nil {
		WriteError(w, nethttp.StatusNotFound, "shares.not_found", "share not found", traceID, nil)
		return
	}
	...
	if share.AccessType == "password" {
		...
		if sessionToken == "" {
			writeJSON(w, traceID, nethttp.StatusOK, sharedThreadResponse{RequiresPassword: true})
			return
		}
		if !validateShareSession(sessionToken, share) {
			WriteError(w, nethttp.StatusForbidden, "shares.invalid_session", "invalid or expired session", traceID, nil)
			return
		}
	}
...
func handleShareVerify(
	...
	if body.Password == "" {
		WriteError(w, nethttp.StatusUnprocessableEntity, "validation.error", "password is required", traceID, nil)
		return
	}
	...
	writeJSON(w, traceID, nethttp.StatusOK, verifyShareResponse{SessionToken: sessionToken})
}
