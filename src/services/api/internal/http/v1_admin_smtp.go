...
func adminSmtpProviderEntry(
	...
	return func(w nethttp.ResponseWriter, r *nethttp.Request) {
		...
		if rawID == "" {
			WriteError(w, nethttp.StatusBadRequest, "validation.error", "id is required", traceID, nil)
			return
		}
		id, err := uuid.Parse(rawID)
		if err != nil {
			WriteError(w, nethttp.StatusBadRequest, "validation.error", "invalid id", traceID, nil)
			return
		}
...
func adminSmtpCreate(
	...
	return func(w nethttp.ResponseWriter, r *nethttp.Request) {
		...
		if err := validateSmtpFields(body.FromAddr, body.SmtpHost, body.SmtpPort, body.TLSMode); err != "" {
			WriteError(w, nethttp.StatusUnprocessableEntity, "validation.error", err, traceID, nil)
			return
		}
		...
		writeJSON(w, traceID, nethttp.StatusCreated, toSmtpProviderResponse(s))
	}
}
...
func adminSmtpUpdateByID(
	...
	return func(w nethttp.ResponseWriter, r *nethttp.Request) {
		...
		if s == nil {
			WriteError(w, nethttp.StatusNotFound, "not_found", "smtp provider not found", traceID, nil)
			return
		}

		writeJSON(w, traceID, nethttp.StatusOK, toSmtpProviderResponse(s))
	}
}
...
func adminSmtpDeleteByID(
	...
	return func(w nethttp.ResponseWriter, r *nethttp.Request) {
		...
		if existing.IsDefault {
			WriteError(w, nethttp.StatusConflict, "conflict", "cannot delete default smtp provider", traceID, nil)
			return
		}
