...
func TestRunsCreateDoesNotAutoResumeWithVisibleFollowupAfterThreadReplacement(t *testing.T) {
	...
	threadResp := doJSON(handler, nethttp.MethodPost, "/v1/threads", map[string]any{"title": "resume after replacement"}, aliceHeaders)
	if threadResp.Code != nethttp.StatusCreated {
		t.Fatalf("create thread: %d body=%s", threadResp.Code, threadResp.Body.String())
	}
	threadPayload := decodeJSONBody[threadResponse](t, threadResp.Body.Bytes())
	...
	if _, err := pool.Exec(ctx,
		...
		t.Fatalf("hide first message: %v", err)
	}
	if _, err := pool.Exec(ctx,
