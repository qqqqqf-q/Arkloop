...
func projectEntry(
	...
	return func(w nethttp.ResponseWriter, r *nethttp.Request) {
		...
		if err != nil {
			WriteError(w, nethttp.StatusUnprocessableEntity, "validation.error", "invalid project id", traceID, nil)
			return
		}
...
func createProject(
	...
	if !requirePerm(actor, auth.PermDataProjectsManage, w, traceID) {
		return
	}
	...
	if visibility != "private" && visibility != "team" && visibility != "org" {
		WriteError(w, nethttp.StatusUnprocessableEntity, "validation.error", "visibility must be private, team, or org", traceID, nil)
		return
	}
	...
	if req.TeamID != nil {
		...
		if err != nil {
			WriteError(w, nethttp.StatusUnprocessableEntity, "validation.error", "invalid team_id", traceID, nil)
			return
		}
		if teamRepo != nil {
			...
			if team == nil || team.OrgID != actor.OrgID {
				WriteError(w, nethttp.StatusUnprocessableEntity, "validation.error", "team not found in org", traceID, nil)
				return
			}
		}
		teamID = &tid
	...
	writeJSON(w, traceID, nethttp.StatusCreated, toProjectResponse(project))
}
...
func listProjects(
	...
	if !requirePerm(actor, auth.PermDataProjectsRead, w, traceID) {
		return
	}
...
func getProject(
	...
	if project == nil || project.OrgID != actor.OrgID {
		WriteError(w, nethttp.StatusNotFound, "projects.not_found", "project not found", traceID, nil)
		return
	}

	writeJSON(w, traceID, nethttp.StatusOK, toProjectResponse(*project))
}
