...
func listAccessLog(
	...
	if rdb == nil {
		WriteError(w, nethttp.StatusServiceUnavailable, "redis.not_configured", "redis not configured", traceID, nil)
		return
	}
	...
	if v := q.Get("risk_min"); v != "" {
		...
		if err != nil || n < 0 || n > 100 {
			WriteError(w, nethttp.StatusUnprocessableEntity, "validation.error", "risk_min must be 0-100", traceID, nil)
			return
		}
		filters.riskMin = n
	}
