...
func subscriptionEntry(
	...
	return func(w nethttp.ResponseWriter, r *nethttp.Request) {
		...
		if err != nil {
			WriteError(w, nethttp.StatusUnprocessableEntity, "validation.error", "invalid subscription id", traceID, nil)
			return
		}
...
func createSubscription(
	...
	if !requirePerm(actor, auth.PermPlatformSubscriptionsManage, w, traceID) {
		return
	}
	...
	if err != nil {
		WriteError(w, nethttp.StatusUnprocessableEntity, "validation.error", "invalid plan_id", traceID, nil)
		return
	}

	periodStart, err := time.Parse(time.RFC3339, strings.TrimSpace(req.CurrentPeriodStart))
	if err != nil {
		WriteError(w, nethttp.StatusUnprocessableEntity, "validation.error", "current_period_start must be RFC3339", traceID, nil)
		return
	}
	periodEnd, err := time.Parse(time.RFC3339, strings.TrimSpace(req.CurrentPeriodEnd))
	if err != nil {
		WriteError(w, nethttp.StatusUnprocessableEntity, "validation.error", "current_period_end must be RFC3339", traceID, nil)
		return
	}

	sub, err := subscriptionRepo.Create(r.Context(), orgID, planID, periodStart, periodEnd)
	if err != nil {
		WriteError(w, nethttp.StatusConflict, "subscriptions.conflict", err.Error(), traceID, nil)
		return
	}

	writeJSON(w, traceID, nethttp.StatusCreated, toSubscriptionResponse(sub))
}
...
func listOrGetSubscription(
	...
	if !requirePerm(actor, auth.PermDataSubscriptionsRead, w, traceID) {
		return
	}
	...
	if sub == nil {
		writeJSON(w, traceID, nethttp.StatusOK, []subscriptionResponse{})
		return
	}
	writeJSON(w, traceID, nethttp.StatusOK, []subscriptionResponse{toSubscriptionResponse(*sub)})
}
...
func getSubscription(
	...
	if sub == nil {
		WriteError(w, nethttp.StatusNotFound, "subscriptions.not_found", "subscription not found", traceID, nil)
		return
	}
	...
	writeJSON(w, traceID, nethttp.StatusOK, toSubscriptionResponse(*sub))
}
...
func cancelSubscription(
	...
	if cancelled == nil {
		WriteError(w, nethttp.StatusNotFound, "subscriptions.not_found", "subscription not found or already cancelled", traceID, nil)
		return
	}

	writeJSON(w, traceID, nethttp.StatusOK, toSubscriptionResponse(*cancelled))
}
