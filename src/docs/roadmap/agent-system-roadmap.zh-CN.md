...
- `pipeline/`：中间件链架构（CancelGuard → InputLoader → Entitlement → MCPDiscovery → AgentConfig → PersonaResolution → Routing → ToolBuild → AgentLoopHandler），各阶段职责独立。
- `personas/`：Persona 注册表，支持文件系统（`src/personas/`）和 DB（`personas` 表）双源加载；`Definition` 包含 id、version、prompt、tool_allowlist、budgets。
...
- Persona 级别的 model/route 绑定（`preferred_credential`）
...
当前所有 Persona 走同一条执行路径（`agent.NewLoop`），只能通过 system_prompt、tool_allowlist、max_iterations 做差异化。但 Lite/Pro/Ultra 的差距远不止参数：
...
**A2 — Persona 定义能力有限，无法绑定执行策略**

`personas.Definition` 只描述"这个 Agent 知道什么"（prompt、tools），没有字段描述"这个 Agent 怎么执行"。新增 Agent 类型 = 修改 Loop 代码。应当让 Persona 声明自己的 `executor_type`，Loop 只是众多 Executor 之一。

**A3 — Model 选择与 Persona/Tier 脱节**

Lite/Pro/Ultra 对应不同的 LLM（haiku/sonnet/opus），但 Persona 定义里没有 `preferred_credential`，`AgentConfig.Model` 虽然存在但从未被路由层读取。Model 的选择完全依赖外部传入 `route_id`，Persona 本身无法声明偏好。
...
`web_search` 的 SearXNG/Tavily 后端、`web_fetch` 的 Jina/Firecrawl/Basic 后端，当前全部通过单一 env var 全局切换，无法做到：per-org 激活不同后端、Console 里可视化管理和配置（API Key、Base URL）、Persona 声明偏好某个特定后端。根本原因是 `AgentToolSpec.Name` 同时承担了"内部注册键"和"LLM 暴露名"两个职责，导致无法区分同一 LLM 名下的多个实现。
...
| AS-2 | Persona 路由绑定 | Persona 声明 `preferred_credential`，接入 `AgentConfig.Model` → 路由层（均按凭证名称匹配） | 顺序 | AS-1 |
| AS-3 | Human-in-the-loop | RunContext 加 WaitForInput 钩子；Ultra executor 可用，Lite 零成本 | 顺序 | AS-1 |
| AS-3.5 | Orchestrator 执行层 | Persona 绑定 AgentConfig；Lua executor；父子 Run 跨实例调度 | 顺序 | AS-1, AS-3 |
...
**目标**：引入 `AgentExecutor` 接口和注册表，将硬编码的 `agent.NewLoop` 调用替换为 dispatch 机制。Persona 通过 `executor_type` 字段声明自己的执行策略。

**AS-1 完成后的效果**：新增任何 Agent 行为（Lite、Pro、Ultra、新的内置任务）只需：
1. 在 `src/personas/agent-xxx/` 写 `persona.yaml`（声明 `executor_type`、`preferred_credential`、`tool_allowlist`）和 `prompt.md`
...
### AS-1.4 — Persona 定义加 executor_type + executor_config

- **修改** `src/services/worker/internal/personas/registry.go`：`Definition` 增加 `ExecutorType string`、`ExecutorConfig map[string]any`。
- **修改** `src/services/worker/internal/personas/loader.go`：解析 YAML 中的 `executor_type`（可选，默认 `"agent.simple"`）和 `executor_config`（可选，默认 `{}`）。
- **验收**：无 `executor_type` 的现有 persona.yaml 加载正常（backward compatible）。

### AS-1.5 — 替换 handler_agent_loop.go 为 dispatch

- **修改** `src/services/worker/internal/pipeline/handler_agent_loop.go`：
  - 从 `rc.PersonaDefinition` 取 `ExecutorType`（默认 `"agent.simple"`）和 `ExecutorConfig`。
...
**目标**：让 Persona 能声明偏好的 LLM route，让 `AgentConfig.Model` 真正参与路由决策，建立完整的 model 选择优先级链。

**解决的问题**：A3

### AS-2.1 — Persona 加 preferred_credential

- **修改** `src/services/worker/internal/personas/registry.go`：`Definition` 增加 `PreferredCredential *string`（凭证显示名称）。
- **修改** `src/services/worker/internal/personas/loader.go`：解析 YAML 字段 `preferred_credential`（可选）；DB 查询列名为 `preferred_credential`。
- **修改** `src/services/worker/internal/pipeline/context.go`：`RunContext` 增加 `PreferredCredentialName string`。
- **修改** `src/services/worker/internal/pipeline/mw_persona_resolution.go`：解析完 Persona 后，若 `def.PreferredCredential != nil`，则设置：

```go
rc.PreferredCredentialName = *def.PreferredCredential
```

**验收**：单测：persona 有 `preferred_credential` 时，`rc.PreferredCredentialName` 正确设置；persona 没有时为空；用户显式传 `route_id` 不受影响。

### AS-2.2 — AgentConfig.Model 接入路由层（已完成）

- `AgentConfig.Model` 和 `Persona.preferred_credential` 均存储凭证显示名称（`llm_credentials.name`），语义统一。
...
InputJSON["route_id"]          ← 用户显式 route ID → Decide() 直接处理
   ↓ 无
Persona.preferred_credential     ← rc.PreferredCredentialName → 凭证名称查找
   ↓ 无匹配
...
**目标**：为 Persona 提供真正的多 Agent 编排能力。三件事必须同时落地才能形成闭环：Persona 显式绑定 AgentConfig（解决模型选择问题）、Lua executor（解决编排逻辑描述问题）、父子 Run 跨实例异步调度（解决执行隔离和性能问题）。
...
### AS-3.5.1 — Persona 绑定 AgentConfig

**目标**：Persona 可以通过 `agent_config` 字段（AgentConfig 名称）显式指定使用哪个 AgentConfig，覆盖继承链解析结果。这是子任务使用不同模型的关键前提。

**当前问题**：Persona 不知道自己的上层 AgentConfig，AgentConfig 由 thread → project → org 继承链自动解析。Ultra persona 内部的 Lua 子任务和主任务会用同一个 AgentConfig，无法让子任务使用更便宜的模型。
...
`src/services/worker/internal/personas/registry.go`：`Definition` 加 `AgentConfigName *string` 字段。

`src/services/worker/internal/personas/loader.go`：YAML 解析 `agent_config` 字段；`LoadFromDB` scan `agent_config_name` 列（需 migration 加列）。

`src/services/api/internal/migrate/migrations/00061_personas_add_agent_config_name.sql`：

```sql
ALTER TABLE personas ADD COLUMN agent_config_name TEXT;
```

`src/services/worker/internal/pipeline/mw_persona_resolution.go`：解析出 `AgentConfigName` 后，若非 nil，执行按名称重新查 AgentConfig 的逻辑，覆盖 `rc.AgentConfig`。
...
**执行顺序**：migration → registry.go → loader.go → mw_persona_resolution.go → mw_agent_config.go 提取辅助函数
...
func SpawnChildRun(ctx context.Context, rc *pipeline.RunContext, personaID string, input string) (string, error)
```
...
```lua
-- Lua 脚本中调用
local result, err = agent.run(persona_id, input_text)
-- 底层映射到 SpawnChildRun
```
...
**目标**：引入 `agent.lua` executor type，Persona 的 `executor_config.script` 字段包含 Lua 脚本，脚本通过 Go binding 访问 Agent 能力，实现可编程编排逻辑。
...
```lua
local tasks = {
  { persona = "lite", input = chunk_1 },
  { persona = "lite", input = chunk_2 },
  { persona = "lite", input = chunk_3 },
}
...
```
AS-3.5.1（Persona 绑定 AgentConfig）
  独立，可最先完成，改动集中在 mw_persona_resolution.go
...
- 需要统一 `agent_id` 约定：默认用 `PersonaDefinition.ID`（并收敛字符集到 `[a-zA-Z0-9_-]`），否则 OpenViking `UserIdentifier` 校验会拒绝。
...
type MemoryIdentity struct {
    ...
    AgentID string    // -> OpenViking agent_id（建议=persona_id）
}
...
- **新建** `src/services/worker/internal/pipeline/mw_memory.go`，插入位置：`PersonaResolutionMiddleware` 之后、`RoutingMiddleware` 之前（保证拿到 `persona_id` 作为 `agent_id`，但不影响路由选择）。
...
- 将 topK 的 `Abstract` 组织成紧凑的 memory block，追加到 `rc.SystemPrompt` 末尾（不覆盖 persona prompt）。
...
- **修改** `src/services/worker/internal/runengine/v1.go`：
  - `MemoryMiddleware` 插入 `PersonaResolutionMiddleware` 之后。
...
- **Pipeline 集成**（`runengine/v1.go`）：MemoryMiddleware 插入 PersonaResolution 之后、Routing 之前。MemoryProvider nil 时整个 middleware no-op。
...
4. 非对话数据是一等公民：用户环境信息、安装的 Persona 列表、Sandbox/Browser 执行结论——不是"对话"但都是 Agent 理解用户的关键上下文。
...
run 结束
  → 收集本次 run 的上下文：
      ...
      - 环境信息变更（新安装的 Persona、Sandbox 环境信息等）
...
| 用户安装新 Persona | entities | persona_{persona_id} | Persona 的 display_name + description | user |
...
**不过度设计**：初期只做"Persona 安装"和"Sandbox 首次执行"两个触发点。其他触发点按需添加，不要一次性全写。
...
2. **tier 确定**：从 `ExecutionContext.Budget` 中读取 `"sandbox_tier"`（由 Persona budgets 声明）。未声明时默认 `"lite"`。
...
**验收**：Browser Service 运行时，persona allowlist 包含 `browser_navigate` 的 agent 可以成功调用；Browser Service 未部署时，Worker 正常启动，browser 工具不出现在 tool 列表中。
...
- **修改** `src/services/worker/internal/pipeline/context.go`：增加 `TokenBudget *int64`（从 Persona budgets 或 AgentConfig 解析）。
- **修改** `src/services/worker/internal/pipeline/mw_persona_resolution.go`：从 `def.Budgets.ToolBudget["max_tokens"]` 读取并写入 `rc.TokenBudget`。
...
- 工具参数：`{"persona_id": "agent-lite", "input": {...}}`。
- 执行逻辑：
...
**验收**：
- Persona YAML 写 `tool_allowlist: [web_search.tavily]`，LLM 收到的 tool spec 名为 `web_search`，执行时走 Tavily executor。
- Persona YAML 写 `tool_allowlist: [web_search]`（旧格式），行为与今天一致（env var 控制后端）。
...
- **Model 优先级链固定**：显式 route_id > Persona.preferred_credential > AgentConfig.Model > Default，不允许其他插入点。
...
- **Persona 绑定 AgentConfig**：Persona 的 `agent_config` 字段（名称）覆盖继承链解析结果；nil 则走原有 thread → project → org → platform 链。绑定在 mw_persona_resolution.go 内完成，mw_agent_config.go 先于其执行，persona 覆盖后不回退。
