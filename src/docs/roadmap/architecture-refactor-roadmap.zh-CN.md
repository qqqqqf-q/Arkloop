...
- `personas/`：从文件系统 `src/personas/` 加载 `persona.yaml + prompt.md`
...
- 先修 Worker 执行路径的结构性问题（MCP 远端化、Personas 入库、凭证入库），再做 Console 管理界面。
...
| Phase 3 | Worker 执行路径重构 | MCP 远端化 + Personas 入库 + 凭证入库 + 事件写入优化 | P0: MCP stdio 绑定; P0: Worker 调度盲目; P1: Personas 绑定文件系统; P1: LLM 凭证锁死 env var; P1: SSE 双重延迟 |
...
- **目标**：`runs` 增加 `parent_run_id`、`status_updated_at`、`completed_at`、`failed_at`、`duration_ms`、`total_input_tokens`、`total_output_tokens`、`total_cost_usd`、`model`、`persona_id`、`deleted_at`。为 `status` 加 CHECK 约束。
- **关键点**：
...
目标：解决 Worker 侧的结构性问题 -- MCP 远端化、Personas 入库、凭证从环境变量迁移到数据库、事件写入路径优化、runs.status 同步更新。
...
- **关键点**：
  ...
  - `model` / `persona_id` 在 `run.started` 时从 `inputJSON` 写入。
...
- **关联审计**：三.3.4「Personas 绑定在文件系统」；P1
- **关联目标架构**：六.6.11.3 `CREATE TABLE personas`
- **目标**：创建 `personas` 表，Personas 从数据库加载（保留文件系统 fallback）。
- **关键点**：
  - `personas` 表字段：`id`、`org_id`（NULL 表示全局 persona）、`persona_key`、`version`、`display_name`、`description`、`prompt_md`、`tool_allowlist`、`budgets_json`、`is_active`、`created_at`。
  - 唯一约束 `(org_id, persona_key, version)`。
  - Worker 的 `personas/loader.go` 保留文件系统加载作为 fallback（内置 persona）；新增从 DB 加载的路径。
  - DB 中的 org 级 persona 覆盖文件系统中的同名 persona。
  - API 侧新增管理端点（面向后续 Console）：
    - `POST /v1/personas`
    - `GET /v1/personas`
    - `PATCH /v1/personas/{id}`
- **具体改动范围**：
  - 新建 migration `00022_create_personas.sql`
  - 新建 `src/services/api/internal/data/personas_repo.go`
  - 新建 `src/services/api/internal/http/v1_personas.go`
  - 修改 `src/services/worker/internal/personas/loader.go`：`LoadFromDB` + fallback 逻辑
- **验收**：
  - `go test -tags integration ./...`
  - 手工：通过 API 创建 persona -> 使用该 persona_id 创建 run -> persona 约束生效。
...
当前 `runengine/v1.go` 的 `Execute` 是一个约 300 行的过程式函数。Phase 1-4 的每次新增能力（MCP 动态加载、per-run 路由、Personas DB 加载）都是往这个函数里追加代码。如果不在此时引入 Pipeline 抽象，Phase 5 的 Agent Config 继承链、Phase 6 的 Content Filter 和 Usage Tracking、以及未来的 Memory System 都将继续膨胀这个函数，最终导致不可维护。
...
- **关键点**：
  ...
  - `RunContext` 字段从现有 Execute 方法中提取，包括但不限于：
    ...
    - `SystemPrompt string`（Persona 或 Agent Config 设置）
...
- **关键点**：
  - 从现有 Execute 中提取以下 Middleware（按执行顺序）：
    ...
    4. **PersonaResolutionMiddleware**：加载 org personas 并解析 persona_id，设置 `RunContext.SystemPrompt`、`RunContext.MaxIterations`、`RunContext.ToolBudget` 等。对应 `personas.ResolvePersona` 段落。
...
- **具体改动范围**：
  ...
  - 新建 `src/services/worker/internal/pipeline/mw_persona_resolution.go`
...
- **关键点**：
  ...
  - `agent_configs`：`id`、`org_id`、`name`、`system_prompt_template_id`、`system_prompt_override`、`model`、`temperature`、`max_output_tokens`、`top_p`、`context_window_limit`、`tool_policy`、`tool_allowlist`、`tool_denylist`、`content_filter_level`、`safety_rules_json`、`project_id`、`persona_id`、`is_default`。
  - `threads` 增加 `agent_config_id`。
  - **Middleware 接入**：新建 `AgentConfigMiddleware` 插入 R46 的 Pipeline（位于 PersonaResolutionMiddleware 之前）。Middleware 按 `run -> thread -> project -> org` 逐级查找 `agent_config`，合并为最终配置写入 `RunContext`（`SystemPrompt`、`MaxIterations`、`MaxOutputTokens`、`ToolPolicy` 等字段）。PersonaResolutionMiddleware 读取 RunContext 中已解析的配置作为基础，persona 的字段覆盖 agent_config 的对应字段。
...
Configuration:  Agent Configs | LLM Credentials | MCP Configs | Personas | Prompt Templates
...
- **关键点**：
  ...
  - 返回字段：id、thread_id、org_id、status、model、persona_id、total_input_tokens、total_output_tokens、total_cost_usd、duration_ms、created_at、completed_at、failed_at。
...
- **目标**：列表 + 创建 + 编辑。
- **关键点**：
  - 列表列：Persona Key、Display Name、Version、Is Active（Badge）、Created At。
  - 创建/编辑 Modal：Persona Key、Display Name、Description（text）、Prompt（textarea）、Tool Allowlist（逗号分隔 text）、Budgets JSON（textarea）、Is Active（toggle）。
- **具体改动范围**：
  - 新建 `src/apps/console/src/pages/personas/PersonasPage.tsx`。
  - 新建 `src/apps/console/src/api/personas.ts`。
...
| Phase 3 | Worker 使用 DB 凭证/MCP 配置/Personas 执行 run；SSE 延迟 < 100ms；runs.status 可信 |
...
| Phase 6.5b | LLM Credentials/Agent Configs/Prompt Templates/MCP Configs/Personas 五个页面 CRUD 功能完整 |
