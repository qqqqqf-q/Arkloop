...
import { updatePlatformSetting } from '../../api-admin'
...
export function ImageGenModelSettingControl({ accessToken, platformSettings, onPlatformSettingsChange, disabled = false }: Props) {
  const { t } = useLocale()
  ...
  const currentValue = platformSettings?.[IMAGE_GENERATIVE_MODEL_KEY] ?? ''
  ...
  const handleChange = async (value: string) => {
    ...
    try {
      if (value === '') {
        ...
        await updatePlatformSetting(accessToken, IMAGE_GENERATIVE_MODEL_KEY, value)
      }
      onPlatformSettingsChange({ [IMAGE_GENERATIVE_MODEL_KEY]: value })
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleTest = async () => {
    if (!imageModelSelection) return
    setTesting(true)
    setTestErrorMenuStyle(null)
    try {
      const result = await testLlmProviderModel(accessToken, imageModelSelection.provider.id, imageModelSelection.model.id)
  ...
  return (
    <div className="flex min-w-0 items-center gap-2">
      ...
      <SettingsIconButton
        ...
        disabled={testing || (!imageModelSelection && !testResult)}
        className="h-9 w-9"
      >
        {testing
