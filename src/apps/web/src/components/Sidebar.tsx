...
import {
  ...
  readGtdEnabled,
  readExpandedProjectPaths,
  writeExpandedProjectPaths,
...
const GTD_BUCKETS: readonly GtdBucket[] = [
  "inbox",
  "todo",
  "waiting",
  "someday",
...
type SidebarDragState =
  ...
  | {
      ...
      x: number;
      y: number;
      sourceBucket: GtdBucket;
    };

function projectGroupLimit(
...
function normalizeGtdBucket(
  value: ThreadResponse["sidebar_gtd_bucket"],
): GtdBucket | null {
  return value && GTD_BUCKETS.includes(value) ? value : null;
}

function withSidebarPatch(
  thread: ThreadResponse,
  patch: UpdateThreadSidebarRequest,
): ThreadResponse {
  ...
  if ("sidebar_pinned" in patch) {
    next.sidebar_pinned_at = patch.sidebar_pinned
    ...
    >
      {isEditing ? (
        ...
        >
          {showStatusDot && (
            <span
              className="flex h-[14px] w-[16px] shrink-0 items-center justify-center"
              aria-hidden="true"
            >
              {isRunning ? (
          ...
          {isStarred && (
            <Star
              size={11}
              className="shrink-0 fill-[var(--c-text-muted)] text-[var(--c-text-muted)] opacity-70"
            />
          )}
          <span className="min-w-0 flex-1 truncate">
            {threadTitle(thread, untitled)}
          </span>
        </button>
      )}
...
export function Sidebar({
  ...
  const {
    ...
    rightPanelOpen: narrow,
  } = useSidebarUI();
  const { openSearchOverlay: onOpenSearchOverlay } = useSearchUI();
  const {
    settingsOpen: suppressActiveThreadHighlight,
  ...
  const [shareModalThreadId, setShareModalThreadId] = useState<string | null>(
    null,
  );
  ...
  const [gtdInboxIds, setGtdInboxIds] = useState<Set<string>>(() =>
    readGtdInboxThreadIds(),
  );
  const [gtdTodoIds, setGtdTodoIds] = useState<Set<string>>(() =>
    readGtdTodoThreadIds(),
  );
  const [gtdWaitingIds, setGtdWaitingIds] = useState<Set<string>>(() =>
    readGtdWaitingThreadIds(),
  );
  const [gtdSomedayIds, setGtdSomedayIds] = useState<Set<string>>(() =>
    readGtdSomedayThreadIds(),
  );
  const [gtdArchivedIds, setGtdArchivedIds] = useState<Set<string>>(() =>
    readGtdArchivedThreadIds(),
  );
  ...
  const [expandedGtdBuckets, setExpandedGtdBuckets] = useState<Set<GtdBucket>>(
    () => defaultGtdExpandedBuckets(),
  );
  const [gtdExpandedLimits, setGtdExpandedLimits] = useState<
    Record<GtdBucket, number>
  >({ inbox: 8, todo: 8, waiting: 8, someday: 8, archived: 8 });
  ...
  const [dragOverProjectPath, setDragOverProjectPath] = useState<string | null>(
    null,
  );
  const [dragOverGtdBucket, setDragOverGtdBucket] = useState<GtdBucket | null>(
    null,
  );
  ...
  const settingsPointerTraceRef =
    useRef<ReturnType<typeof beginPerfTrace>>(null);
  const collapsePointerTraceRef =
    useRef<ReturnType<typeof beginPerfTrace>>(null);
  ...
  useEffect(() => {
    if (!activeThreadId || !completedUnreadThreadIds.has(activeThreadId))
      return;
  ...
  const toggleStar = useCallback(
    (id: string) => {
      ...
      const req = wasStarred
        ? unstarThread(accessToken, id)
        : starThread(accessToken, id);
      req.catch(() => {
        setStarredIds((prev) =>
          wasStarred
            ? [id, ...prev.filter((x) => x !== id)]
  ...
  const patchSidebarState = useCallback(
    (
      id: string,
      patch: UpdateThreadSidebarRequest,
      rollbackLocal?: () => void,
      ...
      void updateThreadSidebarState(accessToken, id, patch)
        .then((updated) => {
  ...
  const projectGroups = useMemo(() => {
    ...
    for (const t of threads) {
      const wf = (
        t.sidebar_work_folder ??
        readThreadWorkFolder(t.id) ??
        ""
      ).trim();
  ...
  const localGtdBucketForThread = useCallback(
    (id: string): GtdBucket | null => {
  ...
  const effectiveGtdBucketForThread = useCallback(
    (thread: ThreadResponse): GtdBucket | null => {
      return (
        normalizeGtdBucket(thread.sidebar_gtd_bucket) ??
        localGtdBucketForThread(thread.id)
      );
    },
  ...
  const gtdGroups = useMemo(() => {
    ...
    for (const thread of threads) {
      ...
      const bucket =
        buckets.find((item) => item.bucket === bucketName) ?? buckets[4];
  ...
  const setProjectGroupNode = useCallback(
  ...
  const setGtdGroupNode = useCallback(
    (bucket: GtdBucket, node: HTMLDivElement | null) => {
      if (node) {
  ...
  const renderThread = useCallback(
    (thread: ThreadResponse, options?: { showStatusDot?: boolean }) => {
      return (
        <SidebarThreadItem
          key={thread.id}
          thread={thread}
  ...
  const ProjectSidebarView = (
    <>
      {projectGroups.map((group) => {
        ...
        const limit = projectGroupLimit(
          expandedLimits[group.path],
        ...
        const toggleExpand = () => {
          ...
          const initialLimit =
            expandedPaths.size > 0
              ? PROJECT_GROUP_SECONDARY_PAGE_SIZE
              : PROJECT_GROUP_PAGE_SIZE;
            ...
            {hasMore && (
              <button
                onClick={() =>
                  setExpandedLimits((prev) => ({
                    ...prev,
                    [group.path]: limit + PROJECT_GROUP_PAGE_SIZE,
                  }))
                }
              ...
              >
                <div
                  className="flex items-center gap-2 text-[13.5px] leading-[20px] text-[var(--c-text-muted)]"
                  style={{ fontWeight: "var(--c-sidebar-thread-weight)" }}
                >
  ...
  const GtdSidebarView = (
    <>
      {gtdGroups.map((group) => {
        ...
        const limit = projectGroupLimit(
          gtdExpandedLimits[group.bucket],
          PROJECT_GROUP_PAGE_SIZE,
        );
        const visible = isExpanded ? group.threads.slice(0, limit) : [];
        const hasMore = isExpanded && group.threads.length > limit;
        ...
        return (
          ...
          >
            ...
            {hasMore && (
              <button
                onClick={() =>
                  setGtdExpandedLimits((prev) => ({
                    ...prev,
                    [group.bucket]: limit + PROJECT_GROUP_PAGE_SIZE,
                  }))
                }
                className="group relative isolate flex h-[34px] w-full items-center rounded-[6px] px-2 py-0 before:pointer-events-none before:absolute before:inset-x-0 before:inset-y-px before:-z-10 before:rounded-[6px] before:content-[''] hover:before:bg-[var(--c-bg-deep)]"
              >
  ...
  // GTD / Pin 操作
  const applyGtdBucketLocal = useCallback(
    (id: string, bucket: GtdBucket | null) => {
      setGtdInboxIds((prev: Set<string>) => {
        const next = new Set(prev);
        if (bucket === "inbox") next.add(id);
        else next.delete(id);
      ...
      setGtdArchivedIds((prev: Set<string>) => {
        const next = new Set(prev);
        if (bucket === "archived") next.add(id);
        else next.delete(id);
  ...
  const currentGtdBucket = useCallback(
    (id: string): GtdBucket | null => {
      const thread = threads.find((item) => item.id === id);
      return thread
        ? effectiveGtdBucketForThread(thread)
        : localGtdBucketForThread(id);
    },
    [effectiveGtdBucketForThread, localGtdBucketForThread, threads],
  );

  const moveGtdThread = useCallback(
    (id: string, bucket: GtdBucket) => {
      const previous = currentGtdBucket(id);
      applyGtdBucketLocal(id, bucket);
      patchSidebarState(id, { sidebar_gtd_bucket: bucket }, () =>
        applyGtdBucketLocal(id, previous),
      );
    },
    [applyGtdBucketLocal, currentGtdBucket, patchSidebarState],
  );

  const markGtdInbox = useCallback(
    (id: string) => moveGtdThread(id, "inbox"),
    [moveGtdThread],
  );
  const markGtdTodo = useCallback(
    (id: string) => moveGtdThread(id, "todo"),
    [moveGtdThread],
  );
  const markGtdWaiting = useCallback(
    (id: string) => moveGtdThread(id, "waiting"),
    [moveGtdThread],
  );
  const markGtdSomeday = useCallback(
    (id: string) => moveGtdThread(id, "someday"),
    [moveGtdThread],
  );
  const archiveGtdThread = useCallback(
    (id: string) => moveGtdThread(id, "archived"),
    [moveGtdThread],
  );
  ...
  const togglePin = useCallback(
    (id: string) => {
      ...
      patchSidebarState(id, { sidebar_pinned: nextPinned }, () =>
        applyPinnedLocal(id, !nextPinned),
      );
    },
  ...
  const pinThread = useCallback(
    (id: string) => {
      ...
      patchSidebarState(id, { sidebar_pinned: true }, () =>
  ...
  const setThreadWorkFolder = useCallback(
    (id: string, folder: string | null) => {
      ...
      patchSidebarState(
        id,
        { sidebar_work_folder: folder === "__unassigned__" ? null : folder },
        () => {
  ...
  const movePinnedThreadToFolder = useCallback(
    (id: string, folder: string | null) => {
      const normalizedFolder = folder === "__unassigned__" ? null : folder;
      const thread = threads.find((item) => item.id === id);
      const previousFolder =
        thread?.sidebar_work_folder ?? readThreadWorkFolder(id);
  ...
  useEffect(() => {
    ...
    const updateDropTargets = (
      e: PointerEvent,
      current: { fromPinned: boolean; sourcePath: string },
    ) => {
      const projectTarget = findProjectTarget(
        e,
        current.sourcePath,
        current.fromPinned,
      );
    ...
    const beginDrag = (e: PointerEvent) => {
      ...
      setDragState({
        ...
        x: e.clientX,
        y: e.clientY,
        fromPinned: candidate.fromPinned,
    ...
    const onPointerDown = (e: PointerEvent) => {
      ...
      candidate = {
        ...
        sourcePath:
          (thread.sidebar_work_folder ?? readThreadWorkFolder(threadId)) ||
          "__unassigned__",
      };
      dragging = null;
    ...
    const onPointerUp = () => {
      ...
      if (dragging) {
        ...
        if (dragging.fromPinned && projectTarget) {
          movePinnedThreadToFolder(
            dragging.id,
            projectTarget === "__unassigned__" ? null : projectTarget,
          );
          ...
          !dragging.fromPinned &&
          projectTarget &&
          projectTarget !== dragging.sourcePath
        ) {
          setThreadWorkFolder(
            dragging.id,
            projectTarget === "__unassigned__" ? null : projectTarget,
  ...
  useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      if (e.key === "Escape") setDeleteConfirmThreadId(null);
    };
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("arkloop:work-folder-changed", handler);
  }, []);
  ...
  useEffect(() => {
    ...
    window.queueMicrotask(() => {
      ...
      if (!areStringSetsEqual(next, pinnedIdsRef.current))
        replacePinnedIds(next);
    });
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "arkloop:sidebar-toggle-started",
      handleToggleStarted as EventListener,
    );
    return () =>
  ...
  useLayoutEffect(() => {
    ...
    if (!current || !isPerfDebugEnabled() || typeof performance === "undefined")
      return;
    const committedAt = performance.now();
    toggleCommittedAtRef.current = committedAt;
    recordPerfValue(
      "sidebar_component_commit",
      committedAt - current.startedAt,
      "ms",
      {
        ...current.sample,
        threadCount: threads.length,
  ...
  useEffect(() => {
    ...
    const handleTransitionStart = (event: TransitionEvent) => {
      ...
      recordPerfValue(
        "sidebar_collapse_transition_start_delay",
        performance.now() - current.startedAt,
        "ms",
        {
          ...current.sample,
      ...
      if (toggleCommittedAtRef.current !== null) {
        recordPerfValue(
          "sidebar_commit_to_transition_start_gap",
          performance.now() - toggleCommittedAtRef.current,
          "ms",
          {
            ...current.sample,
            threadCount: threads.length,
    ...
    return () =>
      aside.removeEventListener("transitionstart", handleTransitionStart);
            ...
            >
              <button
                onClick={() => {
                  ...
                  const currentTitle = thread
                    ? threadTitle(thread, t.untitled)
                    : "";
                  startRename(
                    menuThreadId,
                    currentTitle === t.untitled ? "" : currentTitle,
                  );
                }}
              ...
              >
                <Star
                  ...
                  style={{
                    ...
                    fill: starredIds.includes(menuThreadId)
                      ? "var(--c-text-secondary)"
                      : "none",
                  }}
                />
                {starredIds.includes(menuThreadId)
                  ? t.unstarThread
                  : t.starThread}
              </button>
              <button
              ...
              >
                <Pin
                  ...
                  style={{
                    flexShrink: 0,
                    color: "var(--c-text-secondary)",
                    fill: effectivePinnedIds.has(menuThreadId)
                      ? "var(--c-text-secondary)"
                      : "none",
                  }}
                />
                {effectivePinnedIds.has(menuThreadId)
                  ? t.unpinThread
                  : t.pinThread}
              </button>
  ...
  const navButtonClass =
    "group flex h-[32px] w-full items-center gap-[10px] overflow-hidden whitespace-nowrap rounded-lg px-[8px] text-[13px] text-[var(--c-text-secondary)] transition-colors duration-[60ms] hover:bg-[var(--c-bg-deep)] hover:text-[var(--c-text-primary)]";
      ...
      >
          ...
          (collapsed ? (
            ...
            <div className="flex min-h-[56px] items-center justify-between px-4 py-3">
              <div className="flex items-center gap-2 overflow-hidden">
                <h1 className="text-[16px] font-semibold tracking-tight text-[var(--c-text-primary)] shrink-0">
                  Arkloop
                </h1>
                <div
                  style={{
                    ...
                    transform: isPrivateModeEffective
                      ? "translateX(0)"
                      : "translateX(14px)",
                ...
                >
                  <span
                    className="h-[5px] w-[5px] shrink-0 rounded-full bg-[var(--c-text-tertiary)]"
                    style={{ opacity: 0.5 }}
                  />
                  <span className="text-[12px] font-medium text-[var(--c-text-tertiary)] whitespace-nowrap">
                    {t.incognitoMode}
                  </span>
                </div>
              </div>
              <button
                ...
                onPointerDown={() => {
                  collapsePointerTraceRef.current = beginPerfTrace(
                    "sidebar_collapse_interaction",
                    {
                      phase: "pointerdown",
                      collapsed,
                      threadCount: threads.length,
        ...
        <nav className="flex flex-col items-start gap-px pl-[8px] pr-[7px] pt-1">
          ...
          >
            <span className="flex h-[16px] w-[16px] shrink-0 items-center justify-center">
              <SquarePen
                size={14}
                className="shrink-0 transition-transform duration-100 group-hover:scale-[1.05]"
              />
            </span>
          ...
          <button
            ...
            onPointerDown={() => {
              searchPointerTraceRef.current = beginPerfTrace(
                "sidebar_search_interaction",
                {
                  phase: "pointerdown",
                  collapsed,
                  threadCount: threads.length,
        ...
        >
          ...
          <div className="flex flex-col gap-[2px] flex-1 min-h-0">
            ...
            >
              {threads.length === 0 ? (
                <p className="overflow-hidden whitespace-nowrap px-2 py-1 text-[12px] text-[var(--c-text-muted)]">
                  {t.recentsEmpty}
                </p>
              ) : isWorkMode ? (
                <>
                  ...
                  >
                    ...
                    >
                      ...
                      <span className="ml-1 shrink-0 opacity-0 group-hover/pinned:opacity-100 text-[var(--c-text-muted)] transition-opacity duration-[80ms]">
                        <ChevronRight
                          ...
                          className={[
                            "transition-transform duration-150",
                            pinnedExpanded ? "rotate-90" : "rotate-0",
                          ].join(" ")}
                        />
                      </span>
                    </div>
                    {pinnedExpanded &&
                      pinnedWorkThreads.length === 0 &&
                      renderDropRow(
                    ...
                    {pinnedExpanded &&
                      pinnedWorkThreads.map((th) =>
                        renderThread(th, { showStatusDot: true }),
                      )}
                  </div>
                  {ProjectSidebarView}
                </>
        ...
        <div className="mt-auto px-2 pb-2 pt-1">
          ...
          <div className="mt-0.5 pl-1">
            <button
              ...
              onPointerDown={() => {
                settingsPointerTraceRef.current = beginPerfTrace(
                  "sidebar_settings_interaction",
                  {
                    phase: "pointerdown",
                    collapsed,
                    threadCount: threads.length,
