import { describe, expect, it } from 'vitest'
import { resolveBlockAnimationMeta } from './streamAnimationMeta'

describe('resolveBlockAnimationMeta', () => {
  it('active blocks use the current char delay', () => {
    const result = resolveBlockAnimationMeta({
      currentCharDelay: 18,
      fadeDuration: 280,
      lastElapsedMs: 64,
      previousCharDelay: 12,
      state: 'streaming',
    })

    expect(result.charDelay).toBe(18)
    expect(result.settled).toBe(false)
  })

  it('revealed blocks keep the previous char delay until the fade window ends', () => {
    const result = resolveBlockAnimationMeta({
      currentCharDelay: 10,
      fadeDuration: 280,
      lastElapsedMs: 200,
      previousCharDelay: 20,
      state: 'revealed',
    })

    expect(result.charDelay).toBe(20)
    expect(result.settled).toBe(false)
  })

  it('revealed blocks settle after the fade window', () => {
    const result = resolveBlockAnimationMeta({
      currentCharDelay: 10,
      fadeDuration: 280,
      lastElapsedMs: 320,
      previousCharDelay: 20,
      state: 'revealed',
    })

    expect(result.charDelay).toBe(20)
    expect(result.settled).toBe(true)
  })
})
