...
// per-face fills keyed by face id
const FACE_FILLS: Record<string, { dark: string; light: string }> = {
  top:          { dark: "#FEFEFE", light: "#2A2A2A" },
  left:         { dark: "#EFEFEF", light: "#1A1A1A" },
  "right-upper": { dark: "#DCDCDC", light: "#3A3A3A" },
  blue:         { dark: "#6284FF", light: "#6284FF" },
  "right-lower": { dark: "#D4D4D4", light: "#333333" },
};

const STROKE_COLOR = { dark: "#6284FF", light: "#2A2A2A" };
      ...
      >
        ...
        {FACES.map((face) => (
          <path
            ...
            d={face.d}
            pathLength={1000}
            fill={isFilling ? FACE_FILLS[face.id][mode] : "none"}
            stroke={STROKE_COLOR[mode]}
            strokeWidth="3"
