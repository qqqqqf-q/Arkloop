...
- `spawn_agent` 接收 `persona_id + input`，同步等待子 run 完成
- Lua 侧支持 `agent.run(...)` 和 `agent.run_parallel(...)`
- 父子 run 之间通过 Redis Pub/Sub 传递终态结果

当前主要不足：

- 子 Agent 没有稳定的独立身份，只有 child run
- 返回值只有文本输出，没有结构化状态和后续控制句柄
- 没有 `send_input` / `wait` / `resume` / `close` 等控制原语
...
- `agent.spawn(...) -> { id, status }`
- `agent.send(id, input)`
- `agent.wait(ids, timeout_ms)`
- `agent.close(id)`
- `agent.resume(id)`

#### 高层语法糖

- `agent.run(persona, input)` = `spawn + wait + collect`
- `agent.run_parallel(tasks)` = `spawn N 个 + wait + collect`

这样未来如果要增加长生命周期子 Agent，不需要推翻现有 Lua API。
