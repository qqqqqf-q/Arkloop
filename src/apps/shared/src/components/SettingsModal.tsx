...
export type SettingsModalTranslations = {
  account: string
  settings: string
  signOut: string
  language: string
...
type Props = {
  ...
  t: SettingsModalTranslations
}

export function SettingsModal({ me, onClose, onLogout, brandLabel, roleLabel, locale, setLocale, t }: Props) {
      ...
      >
        ...
        <div className="flex flex-1 flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-6">
            {active === 'account' && (
              <AccountTab me={me} userInitial={userInitial} roleLabel={roleLabel} onLogout={() => { onLogout(); onClose() }} signOutLabel={t.signOut} />
            )}
            {active === 'settings' && <SettingsTab locale={locale} setLocale={setLocale} t={t} />}
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="flex flex-col gap-3">
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          {roleLabel && (
            <div className="text-xs text-[var(--c-text-muted)]">{roleLabel}</div>
          )}
        </div>
        <button
          onClick={onLogout}
